...
from helpers import apology, login_required, lookup, apicall#, usd
...
def search():
    ...
    if request.method == "POST":
        ...
        if not request.form.get("ip"):
            return apology("missing ip")

        # Get IP address
        search = apicall(request.form.get("ip"))
        print("Scan results: %s" % search)
        # if not quote:
        #     return apology("invalid symbol")
